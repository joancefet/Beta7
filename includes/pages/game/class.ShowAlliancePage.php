...
class ShowAlliancePage extends AbstractPage
{
	...
	{
	global $ALLIANCE,$UNI,$USER;
	
		$searchList	= array();

			$searchResult = $GLOBALS['DATABASE']->query("SELECT 
			id, ally_name, ally_tag, ally_members,FraLevel,fraction, ally_max_members, ally_image
			FROM ".ALLIANCE."
			...
			while($searchRow = $GLOBALS['DATABASE']->fetch_array($searchResult))
			{
				$searchList[]	= array(
					'id'		=> $searchRow['id'],
					'fraction'		=> $searchRow['fraction'],
					'level'		=> $searchRow['FraLevel'],
	...
	function search() 
	{
		...
		{
			$searchResult = $GLOBALS['DATABASE']->query("SELECT 
			id, ally_name, ally_tag, ally_members, ally_max_members, ally_image
			FROM ".ALLIANCE."
...
/* 	function vlyat() 
	...
	$storages  = $GLOBALS['DATABASE']->fetch_array($storages);
		...
		$this->tplObj->assign_vars(array(
				'Userlist'			=> $Userlist,
				'storage_metal'			=> shortly_number($storages['storage_metal']),
				'storage_crystal'			=> shortly_number($storages['storage_crystal']),
				'storage_deuterium'			=> shortly_number($storages['storage_deuterium']),
				'storage_metal1'			=> $storages['storage_metal'],
				'storage_crystal1'			=> $storages['storage_crystal'],
				'storage_deuterium1'			=> $storages['storage_deuterium'],
		));
	...
	function issuesend() 
	{
		...
		$GLOBALS['DATABASE']->query("UPDATE ".PLANETS." set `metal` = metal + ".$metal." where `id` = '".$userid."';");
		$GLOBALS['DATABASE']->query("UPDATE ".PLANETS." set `crystal` = crystal + ".$crystal." where `id` = '".$userid."';");
		$GLOBALS['DATABASE']->query("UPDATE ".PLANETS." set `deuterium` = deuterium + ".$deuterium." where `id` = '".$userid."';");
		
		
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `storage_metal` = `storage_metal` - ".$metal." where `id` = '".$this->allianceData['id']."';");
        $GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `storage_crystal` = `storage_crystal` - ".$crystal." where `id` = '".$this->allianceData['id']."';");
        $GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `storage_deuterium` = `storage_deuterium` - ".$deuterium." where `id` = '".$this->allianceData['id']."';");
		$GLOBALS['DATABASE']->query("INSERT INTO `uni1_storages_logs` VALUES (".$GLOBALS['DATABASE']->GetInsertID().",".$this->allianceData['id'].",".$USER['id'].",".$metal.",".$crystal.",".$deuterium.", ".TIMESTAMP.", '2') ;");
		...
		$this->printMessage("Ressource have been added to the choosen player");
	}
	}
	...
	function development() 
	{
		...
		$alliance_prod_price = 3 + ($this->allianceData['alliance_prod'] * 4);
		$alliance_fleet_speed_price = 3 + ($this->allianceData['alliance_fleet_speed'] * 4);
		$alliance_battle_tech_price = 3 + ($this->allianceData['alliance_battle_tech'] * 6);
		$alliance_build_speed_price = 1 + ($this->allianceData['alliance_build_speed'] * 2);
		$alliance_research_speed_price = 1 + ($this->allianceData['alliance_research_speed'] * 2);
		$alliance_fleet_construction_price = 1 + ($this->allianceData['alliance_fleet_construction'] * 2);
		$alliance_def_construction_price = 1 + ($this->allianceData['alliance_def_construction'] * 2);
        ...
        switch($con){
        case 'alliance_prod':
		if($this->allianceData['alliance_points'] >= $alliance_prod_price){
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `alliance_prod` = `alliance_prod` + '1', `alliance_points` = `alliance_points` - ".$alliance_prod_price." where `id` = '".$this->allianceData['id']."' ;");
		$this->printMessage("Alliance production speed has been upgraded!", true, array('game.php?page=alliance&mode=development', 2));
		echo json_encode(array('success' => true));
		die();
		}else{
		$this->printMessage("There are not enough alliance points!", true, array('game.php?page=alliance&mode=development', 2));
		die();
		}		
        break;
		case 'alliance_fleet_speed':
		if($this->allianceData['alliance_points'] >= $alliance_fleet_speed_price){
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `alliance_fleet_speed` = `alliance_fleet_speed` + '1', `alliance_points` = `alliance_points` - ".$alliance_fleet_speed_price." where `id` = '".$this->allianceData['id']."' ;");
		$this->printMessage("Alliance fleet speed has been upgraded!", true, array('game.php?page=alliance&mode=development', 2));
		echo json_encode(array('success' => true));
		die();
		}else{
		$this->printMessage("There are not enough alliance points!", true, array('game.php?page=alliance&mode=development', 2));
        ...
        break;
		case 'alliance_battle_tech':
		if($this->allianceData['alliance_points'] >= $alliance_battle_tech_price){
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `alliance_battle_tech` = `alliance_battle_tech` + '1', `alliance_points` = `alliance_points` - ".$alliance_battle_tech_price." where `id` = '".$this->allianceData['id']."' ;");
		$this->printMessage("Alliance battle specs is upgraded!", true, array('game.php?page=alliance&mode=development', 2));
		echo json_encode(array('success' => true));
		die();
		}else{
		$this->printMessage("There are not enough alliance points!", true, array('game.php?page=alliance&mode=development', 2));
        ...
        break;
		case 'alliance_build_speed':
		if($this->allianceData['alliance_points'] >= $alliance_build_speed_price){
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `alliance_build_speed` = `alliance_build_speed` + '1', `alliance_points` = `alliance_points` - ".$alliance_build_speed_price." where `id` = '".$this->allianceData['id']."' ;");
		$this->printMessage("Alliance build construction time has been reduced!", true, array('game.php?page=alliance&mode=development', 2));
		echo json_encode(array('success' => true));
		die();
		}else{
		$this->printMessage("There are not enough alliance points!", true, array('game.php?page=alliance&mode=development', 2));
        ...
        break;
		case 'alliance_research_speed':
		if($this->allianceData['alliance_points'] >= $alliance_research_speed_price){
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `alliance_research_speed` = `alliance_research_speed` + '1', `alliance_points` = `alliance_points` - ".$alliance_research_speed_price." where `id` = '".$this->allianceData['id']."' ;");
		$this->printMessage("Alliance research time has been reduced!", true, array('game.php?page=alliance&mode=development', 2));
		echo json_encode(array('success' => true));
		die();
		}else{
		$this->printMessage("There are not enough alliance points!", true, array('game.php?page=alliance&mode=development', 2));
        ...
        break;
		case 'alliance_fleet_construction':
		if($this->allianceData['alliance_points'] >= $alliance_fleet_construction_price){
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `alliance_fleet_construction` = `alliance_fleet_construction` + '1', `alliance_points` = `alliance_points` - ".$alliance_fleet_construction_price." where `id` = '".$this->allianceData['id']."' ;");
		$this->printMessage("Alliance fleets construction time has been reduced!", true, array('game.php?page=alliance&mode=development', 2));
		echo json_encode(array('success' => true));
		die();
		}else{
		$this->printMessage("There are not enough alliance points!", true, array('game.php?page=alliance&mode=development', 2));
        ...
        break;
		case 'alliance_def_construction':
		if($this->allianceData['alliance_points'] >= $alliance_def_construction_price){
		$GLOBALS['DATABASE']->query("UPDATE ".ALLIANCE." set `alliance_def_construction` = `alliance_def_construction` + '1', `alliance_points` = `alliance_points` - ".$alliance_def_construction_price." where `id` = '".$this->allianceData['id']."' ;");
		$this->printMessage("Alliance defense construction time has been reduced!", true, array('game.php?page=alliance&mode=development', 2));
		echo json_encode(array('success' => true));
		die();
		}else{
		$this->printMessage("There are not enough alliance points!", true, array('game.php?page=alliance&mode=development', 2));
		...
		$this->tplObj->assign_vars(array(
		'alliance_prod' => $this->allianceData['alliance_prod'],
		'alliance_fleet_speed' => $this->allianceData['alliance_fleet_speed'],
		'alliance_battle_tech' => $this->allianceData['alliance_battle_tech'],
		'alliance_build_speed' => $this->allianceData['alliance_build_speed'],
		'alliance_research_speed' => $this->allianceData['alliance_research_speed'],
		'alliance_fleet_construction' => $this->allianceData['alliance_fleet_construction'],
		'alliance_def_construction' => $this->allianceData['alliance_def_construction'],
		'alliance_points' => $this->allianceData['alliance_points'],
		'alliance_prod_price' => $alliance_prod_price,
		'alliance_fleet_speed_price' => $alliance_fleet_speed_price,
		'alliance_battle_tech_price' => $alliance_battle_tech_price,
		'alliance_build_speed_price' => $alliance_build_speed_price,
		'alliance_research_speed_price' => $alliance_research_speed_price,
		'alliance_fleet_construction_price' => $alliance_fleet_construction_price,
		'alliance_def_construction_price' => $alliance_def_construction_price,
 ...
 function fraction()
	...
	global $ALLIANCE;
		...
		$this->tplObj->assign_vars(array(
			'fraction'		=> $this->allianceData['fraction'],
			'level'			=> $this->allianceData['FraLevel'],
			'armament'  	=> $this->allianceData['armament'],
			'armamentDA'  	=> $this->allianceData['armamentDA'],
			'Bumper'  		=> $this->allianceData['Bumper'],
			'Boards'  		=> $this->allianceData['Boards'],
			'SpeedFleet'  	=> $this->allianceData['SpeedFleet'],
			'TheftResource' => $this->allianceData['TheftResource'],
			'CombatExp'  	=> $this->allianceData['CombatExp'],
			'LoseDefWreck'  => $this->allianceData['LoseDefWreck'],
			'RestorationDef'=> $this->allianceData['RestorationDef'],
			'FleetCapa'=> $this->allianceData['FleetCapa'],
			'FuelReduce'=> $this->allianceData['FuelReduce'],
			'ComExpExpo'=> $this->allianceData['ComExpExpo'],
			'GetAlliancePoints'=> $this->allianceData['GetAlliancePoints'],
			'ResourceProduk'=> $this->allianceData['ResourceProduk'],
			'EnergyProduk'=> $this->allianceData['EnergyProduk'],
			'PeaceExp'=> $this->allianceData['ExpPeace'],
			'SpeedExpo'=> $this->allianceData['SpeedExpo'],
			''
		));	

		if($fraction == 1){
		$this->display('page.alliance.fraction.tpl');
		}elseif($fraction == 2){
