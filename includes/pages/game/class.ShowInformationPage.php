...
class ShowInformationPage extends AbstractPage
{
	...
	static function getNextJumpWaitTime($lastTime, $spruntor)
	{
		global $CONF, $PLANET, $resource;
		
		return $lastTime + Config::get('gate_wait_time') / $spruntor;
	}
	...
	public function sendFleet()
	{
		...
		$TargetGate   = $GLOBALS['DATABASE']->getFirstRow("SELECT id, last_jump_time, sprungtor FROM ".PLANETS." WHERE id = ".$TargetPlanet." AND id_owner = ".$USER['id']." AND sprungtor > 0;");
