...
class ShowBanListPage extends AbstractPage 
{
	...
	function pmList()
	{
		...
		$banCount	= $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM uni1_message_banned WHERE universe = ".$UNI." ORDER BY time DESC;");
		
		$maxPage	= ceil($banCount / BANNED_USERS_PER_PAGE);
		$page		= max(1, min($page, $maxPage));
		
		$banResult	= $GLOBALS['DATABASE']->query("SELECT * FROM uni1_message_banned WHERE universe = ".$UNI." ORDER BY time DESC LIMIT ".(($page - 1) * BANNED_USERS_PER_PAGE).", ".BANNED_USERS_PER_PAGE.";");
		$banList	= array();
		...
		$this->display('page.banList.message.tpl');
	}
