...
class ShowFleetStep2Page extends AbstractPage
{
	...
	public function show()
	{
		...
		$isAllowedFortress = $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM uni1_planets WHERE id_owner = ".$USER['id']." AND planet_type = '4';");
		...
		if($targetType == 4){
		$targetPlanetData			= $GLOBALS['DATABASE']->getFirstRow("SELECT `id`, `planet_protection`, `id_owner`, `planet_type` FROM `".PLANETS."` WHERE `universe` = ".$UNI." AND `galaxy` = ".$targetGalaxy." AND `system` = ".$targetSystem." AND `planet` = ".$targetPlanet." AND `planet_type` = '4';");
		}else{
		$targetPlanetData			= $GLOBALS['DATABASE']->getFirstRow("SELECT `id`, `planet_protection`, `id_owner`, `planet_type`, `der_metal`, `der_crystal` FROM `".PLANETS."` WHERE `universe` = ".$UNI." AND `galaxy` = ".$targetGalaxy." AND `system` = ".$targetSystem." AND `planet` = ".$targetPlanet." AND `planet_type` = '1';");
		}
		...
		if($targetType == 4 && $isAllowedFortress >= 3)
		{
			$this->printMessage('You can have a maximum of 3 fortress under your hands');
		}
		...
		if($fleetEndTime - 6*60*60 >= $fleetStartTime && $targetPlanetData['id_owner'] != $USER['id'] && $targetMission != 7){
		$xpx = 1;
		}
