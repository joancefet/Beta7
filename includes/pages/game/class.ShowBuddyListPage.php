...
class ShowBuddyListPage extends AbstractPage
{
	...
	function request()
	{
		...
		$ennemieexists	= $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM uni1_ennemies WHERE (sender = ".$USER['id']." AND owner = ".$id.");");
		...
		if($ennemieexists != 0)
		{
			$this->printMessage('Delete the player first from your ennemie list');
		}
