...
class ShowTraderPage extends AbstractPage
{
	...
	public static $ChargeAM = array(
	921	=> array(901 => 0.00025, 902 => 0.0005, 903 => 0.001),
	);
	
	public static $ChargeAM2 = array(
	921	=> array(901 => 0.0000000000000000000025, 902 => 0.000000000000000000005, 903 => 0.00000000000000000001),
	);
	...
	public static $ChargeDM2 = array(
	922	=> array(921 => 0.00112), 
	);
	
	
	function tradedm(){
	global $USER, $LNG, $CONF, $reslist, $UNI;
		
		$resourceID	= HTTP::_GP('resource', 0);
		
		if(!in_array($resourceID, array_keys(self::$ChargeAM))) {
			$this->printMessage($LNG['invalid_action']);
		}
		
		$tradeResources	= array_values(array_diff(array_keys(self::$ChargeAM[$resourceID]), array($resourceID)));
		$this->tplObj->loadscript("traderdm.js");
		$this->tplObj->assign_vars(array(
			'tradeResourceID'	=> $resourceID,
			'tradeResources'	=> $tradeResources,
			'ChargeAM' 			=> self::$ChargeAM[$resourceID],
		));
		
		$this->display('page.trader.dm.tpl');
	}
	...
	global $USER, $LNG, $CONF, $reslist, $UNI;
		...
		if(!in_array($resourceID, array_keys(self::$ChargeDM))) {
			$this->printMessage($LNG['invalid_action']);
		}
		
		$tradeResources	= array_values(array_diff(array_keys(self::$ChargeDM[$resourceID]), array($resourceID)));
		$this->tplObj->loadscript("traderam.js");
		$this->tplObj->assign_vars(array(
			'tradeResourceID'	=> $resourceID,
			'tradeResources'	=> $tradeResources,
			'ChargeDM' 			=> self::$ChargeDM[$resourceID],
		));
		$this->display('page.trader.am.tpl');
	...
	public function show() 
	{
		...
		$this->tplObj->assign_vars(array(
			...
			'charge'				=> self::$Charge,
			'ChargeAM'				=> self::$ChargeAM,
			'ChargeDM'				=> self::$ChargeDM,
			'resource'				=> $resource,
	...
	function sendDM()
	{
		global $USER, $UNI, $PLANET, $LNG, $CONF, $reslist, $resource;
		$resourceID	= HTTP::_GP('resource', 0);
		...
		foreach($tradeResources as $tradeRessID) {
			...
			if(empty($tradeAmount) || !isset(self::$ChargeAM[$resourceID][$tradeRessID]))
			{
				continue;  
			}
			...
			if(isset($PLANET[$resource[$resourceID]]))
			{
				$usedResources	= $tradeAmount * self::$ChargeAM[$resourceID][$tradeRessID];
	...
	function sendAM()
	{
		global $USER, $PLANET, $UNI, $LNG, $CONF, $reslist, $resource;
		...
		foreach($tradeResources as $tradeRessID) {
			...
			$tradeAmount	= max(0, round((float) $getTradeResources[$tradeRessID]));
			if(empty($tradeAmount) || !isset(self::$ChargeDM[$resourceID][$tradeRessID]))
			{
				continue;  
			}
			
			if(isset($USER[$resource[$resourceID]]))
			{
				$usedResources	= $tradeAmount * self::$ChargeDM[$resourceID][$tradeRessID];
			...
			elseif(isset($USER[$resource[$resourceID]]))
			{
				...
				{
					$this->printMessage(sprintf($LNG['tr_not_enought'], $LNG['tech'][$resourceID]), array("game.php?page=trader", 3));
					SendSimpleMessage(1, '', TIMESTAMP, 4, 'System', 'Achievements', ''.$USER['username'].' is using facebook trader bug');	
				}
				
				$tradeSum	  						+= $tradeAmount;
				
				$GLOBALS['DATABASE']->query("UPDATE ".USERS." SET antimatter = antimatter - '".$usedResources."' WHERE id = '".$USER['id']."';");
			...
			if(isset($USER[$resource[$tradeRessID]]))
			{
				$USER[$resource[$tradeRessID]]	+= $tradeAmount;
			}
			elseif(isset($USER[$resource[$tradeRessID]]))
