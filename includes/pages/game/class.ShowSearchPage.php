...
class ShowSearchPage extends AbstractPage
{
	...
	static function _getSearchList($seachMode, $searchText, $maxResult)
	{
		...
		switch($seachMode) {
			...
			$listed = 1;
			if($PLANET['searcher'] > 2){
			$listed = floor($PLANET['searcher'] / 2) +1;
			}

			if($listed > 1){
				$searchResult = $GLOBALS['DATABASE']->query("SELECT 
				a.id, a.username, a.planet_cloak, a.ally_id, b.galaxy, b.planet_type, b.system, b.planet, b.name, c.total_rank, d.ally_name
				FROM ".USERS." as a 
				...
				WHERE b.planet_type = '1' AND a.universe = '".$UNI."' AND a.username LIKE '%".$GLOBALS['DATABASE']->sql_escape($searchText, true)."%'
				ORDER BY (
				...
				) DESC, b.galaxy, b.system, b.planet, b.planet_type ASC LIMIT ".$listed.";");
				}else{
				$searchResult = $GLOBALS['DATABASE']->query("SELECT 
				a.id, a.planet_cloak, a.username, a.ally_id, a.galaxy, a.system, a.planet, b.name, c.total_rank, d.ally_name
				FROM ".USERS." as a 
				...
				{
					$searchList[]	= array(
						'planet_cloak'	=> $searchRow['planet_cloak'],
						'TIMER'	=> TIMESTAMP,
			...
			case 'planetname':
				...
				b.id, b.planet_cloak, b.ally_id, b.username, 
				c.total_rank, 
				d.ally_name 
				FROM ".PLANETS." as a 
