...
class ShowMessagesPage extends AbstractPage
{
	...
	function action()
	{
		...
		switch($action)
		{
			...
			break;
			case 'deleteall':
				$GLOBALS['DATABASE']->query("DELETE FROM ".MESSAGES." WHERE message_owner = ".$USER['id']." AND message_type != '200';");
			break;
			case 'deletetypeall':
			...
			case 'archivemarked':
				...
				$GLOBALS['DATABASE']->query("UPDATE ".MESSAGES." SET message_type = '200' WHERE message_id IN (".implode(',', array_keys($messageIDs)).") AND message_owner = ".$USER['id'].";");
			break;
			case 'deleteunmarked':
	...
	function show()
	{
		...
		$CategoryType   = array ( 0, 1, 2, 3, 4, 5, 15, 50, 99, 100, 200, 999);
