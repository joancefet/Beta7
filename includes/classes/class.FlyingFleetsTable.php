...
class FlyingFleetsTable
{
	...
	function getFleets($acsID = false) {
		...
		if($this->IsPhalanx) {
			$SQLWhere	= "(fleet_start_id = ".$this->PlanetID." AND fleet_mission != 4) OR (fleet_end_id = ".$this->PlanetID." AND fleet_mess = 0) OR (fleet_end_id = ".$this->PlanetID." AND fleet_mess = 2)";
		} elseif(!empty($acsID)) {
