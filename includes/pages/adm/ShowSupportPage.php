...
class ShowSupportPage
{
	...
	public function show()
	{
		...
		$ticketResult	= $GLOBALS['DATABASE']->query("SELECT t.*, u.username, COUNT(a.ticketID) as answer FROM ".TICKETS." t INNER JOIN ".TICKETS_ANSWER." a USING (ticketID) INNER JOIN ".USERS." u ON u.id = t.ownerID WHERE t.universe >= '1' GROUP BY a.ticketID ORDER BY t.status ASC;");
		$ticketList		= array();
	...
	function send() 
	{
		...
		SendSimpleMessage($ticketDetail['ownerID'], $USER['id'], TIMESTAMP, 4, $USER['username'], sprintf($LNG['sp_answer_message_title'], $subject), sprintf($LNG['sp_answer_message'], $ticketID, $subject)); 
