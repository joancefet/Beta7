...
class ShowBonusPage extends AbstractPage
{
	...
	function gift1(){
		...
		if($USER['frisbee'] >= 1){
			$am = mt_rand(50,60);
			$USER['antimatter'] += $am;
			$GLOBALS['DATABASE']->query("UPDATE ".USERS." SET `antimatter` = `antimatter` + ".$am.", `frisbee` = `frisbee` - 1 WHERE `id` = ".$USER['id'].";");
			$this->printMessage("Você obteve <br> ".$am." Anti Matéria", true, array('game.php?page=overview', 2));
		}else{
	...
	function gift2(){
		...
		if($USER['alien'] >= 1){
			$am = mt_rand(50,60);
			$USER['antimatter'] += $am;
			$GLOBALS['DATABASE']->query("UPDATE ".USERS." SET `antimatter` = `antimatter` + ".$am.", `alien` = `alien` - 1 WHERE `id` = ".$USER['id'].";");
			$this->printMessage("Você obteve <br> ".$am." Anti Matéria", true, array('game.php?page=overview', 2));
		}else{
	...
	function gift3(){
		...
		if($USER['rocket'] >= 1){
			$am = mt_rand(50,60);
			$USER['antimatter'] += $am;
			$GLOBALS['DATABASE']->query("UPDATE uni1_users SET `antimatter` = `antimatter` + ".$am.", `rocket` = `rocket` - 1 WHERE `id` = ".$USER['id'].";");
			$this->printMessage("Você obteve <br> ".$am." Anti Matéria", true, array('game.php?page=overview', 2));
		}else{
	...
	function gift4(){
		...
		if($get_extra > $get_extra_false){
			...
			$GLOBALS['DATABASE']->query("Update uni1_users SET `antimatter` = `antimatter` + ".$am.", `frisbee` = `frisbee` - '1', `alien` = `alien` - '1', `rocket` = `rocket` - '1' WHERE `id` = ".$USER['id'].";");
			$GLOBALS['DATABASE']->query("Update uni1_users SET ".$varis." = ".$varis." + ".$valuee." WHERE `id` = ".$USER['id'].";");
			$this->printMessage("Você obteve ".$am." Anti Matéria + ".$Message."", true, array('game.php?page=overview', 2));
		}else{
			$GLOBALS['DATABASE']->query("Update uni1_users SET `antimatter` = `antimatter` + ".$am.", `frisbee` = `frisbee` - '1', `alien` = `alien` - '1', `rocket` = `rocket` - '1' WHERE `id` = ".$USER['id'].";");
			$this->printMessage("Você obteve ".$am." Anti Matéria", true, array('game.php?page=overview', 2));
		}
	}
