...
class ShowMoonPhalanxPage extends AbstractPage
{
	...
	static function allowPhalanx($toGalaxy, $toSystem)
	{
		...
		if ($PLANET['galaxy'] != $toGalaxy || $PLANET[$resource[46]] == 0 || !isModulAvalible(MODULE_PHALANX) || $PLANET[$resource[903]] < PHALANX_DEUTERIUM) {
			return false;
		}
		
		$PhRange	= self::GetPhalanxRange($PLANET[$resource[46]]);
	...
	{
		return ($PhalanxLevel == 1) ? 1 : pow($PhalanxLevel, 3) - 1;
	}
	...
	function show()
	{
		...
		$TargetInfo = $GLOBALS['DATABASE']->getFirstRow("SELECT id, name, id_owner FROM ".PLANETS." WHERE `universe` = '".$UNI."' AND `galaxy` = '".$Galaxy."' AND `system` = '".$System."' AND `planet` = '".$Planet."' AND `planet_type` = '3';");
		...
		$this->display('page.moonphalanx.default.tpl');			
	}
}