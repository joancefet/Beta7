...
class GalaxyRows
{
	...
	public function getGalaxyData()
	{
		...
		p.galaxy, p.system, p.planet, p.capture_not, p.planet_protection, p.planet_type, p.id, p.id_owner, p.name, p.image, p.last_update, p.diameter, p.temp_min, p.destruyed, p.der_metal, p.der_crystal, p.id_luna, 
		u.id as userid, u.ally_id, u.username, u.planet_cloak, u.onlinetime, u.urlaubs_modus, u.immunity_until, u.banaday, 
		...
		a.id as allyid, a.fraction, a.ally_tag, a.ally_web, a.ally_members, a.ally_name, 
		allys.total_rank as ally_rank,
		COUNT(buddy.id) as buddy,
		COUNT(uni1_ennemies.id) as ennemies,
		...
		LEFT JOIN uni1_ennemies ON (uni1_ennemies.sender = ".$USER['id']." AND uni1_ennemies.owner = u.id )
		WHERE p.universe = ".$UNI." AND p.galaxy = ".$this->Galaxy." AND p.system = ".$this->System." AND p.planet_type != '3'
		GROUP BY p.id;");
	...
	protected function setLastActivity()
	{
		...
		if ($this->galaxyRow['planet_type'] == 4) {
			$this->galaxyData[$this->galaxyRow['planet']]['lastActivity']	= '';	
		} elseif ($lastActivity < 4) {
	...
	protected function isFortress()
	{
		global $PLANET, $USER;
		
		$this->galaxyData[$this->galaxyRow['planet']]['isFortress']	= $this->galaxyRow['planet_type'] == 4;
	}
	
	protected function isFortressTimer()
	{
		global $PLANET, $USER;
		$this->galaxyData[$this->galaxyRow['planet']]['isFortressTimer']	= ((!empty($this->galaxyRow['capture_not']) && $this->galaxyRow['capture_not'] > TIMESTAMP) ? ($this->galaxyRow['capture_not'] - TIMESTAMP) : 0);
	}
	...
	protected function getAllowedMissions()
	{
		...
		1	=> !$this->galaxyData[$this->galaxyRow['planet']]['ownPlanet'] && $USER['immunity_until'] < TIMESTAMP && isModulAvalible(MODULE_MISSION_ATTACK) && $this->galaxyRow['id_owner'] != Asteroid_Id && $this->galaxyRow['planet_type'] != 4 && $this->galaxyRow['planet_protection'] < TIMESTAMP,
		3	=> isModulAvalible(MODULE_MISSION_TRANSPORT) && $this->galaxyRow['id_owner'] != Asteroid_Id && $this->galaxyRow['planet_type'] != 4 || isModulAvalible(MODULE_MISSION_TRANSPORT) && $this->galaxyRow['id_owner'] == $USER['id'] && $this->galaxyRow['planet_type'] == 4,
		4	=> $this->galaxyData[$this->galaxyRow['planet']]['ownPlanet'] && $USER['immunity_until'] < TIMESTAMP && isModulAvalible(MODULE_MISSION_STATION) && $this->galaxyRow['id_owner'] != Asteroid_Id && $this->galaxyRow['planet_type'] != 4,
		5	=> !$this->galaxyData[$this->galaxyRow['planet']]['ownPlanet'] && $USER['immunity_until'] < TIMESTAMP && isModulAvalible(MODULE_MISSION_HOLD) && $this->galaxyRow['id_owner'] != Asteroid_Id && $this->galaxyRow['planet_type'] != 4,
		6	=> !$this->galaxyData[$this->galaxyRow['planet']]['ownPlanet'] && $USER['immunity_until'] < TIMESTAMP && isModulAvalible(MODULE_MISSION_SPY) && $this->galaxyRow['id_owner'] != Asteroid_Id && $this->galaxyRow['planet_type'] != 4,
		8	=> isModulAvalible(MODULE_MISSION_RECYCLE) && $this->galaxyRow['id_owner'] != Asteroid_Id && $this->galaxyRow['planet_type'] != 4,
		9	=> !$this->galaxyData[$this->galaxyRow['planet']]['ownPlanet'] && $PLANET[$resource[214]] > 0 && isModulAvalible(MODULE_MISSION_DESTROY) && $this->galaxyRow['id_owner'] != Asteroid_Id && $USER['rpg_destructeur'] > 0 && $USER['immunity_until'] < TIMESTAMP && $this->galaxyRow['planet_type'] != 4,
		10	=> !$this->galaxyData[$this->galaxyRow['planet']]['ownPlanet'] && $PLANET[$resource[503]] > 0 && isModulAvalible(MODULE_MISSION_ATTACK) && isModulAvalible(MODULE_MISSILEATTACK) && $this->inMissileRange() && $this->galaxyRow['id_owner'] != Asteroid_Id && $this->galaxyRow['planet_type'] != 4,
		12	=> $this->galaxyRow['id_owner'] == Asteroid_Id && $USER['immunity_until'] < TIMESTAMP && $this->galaxyRow['planet_type'] != 4,
		20	=> $this->galaxyRow['planet_type'] == 4 && $this->galaxyRow['id_owner'] == Fortress_Id,
		);
	}
	...
	protected function getActionButtons()
	{
        ...
        if($this->galaxyData[$this->galaxyRow['planet']]['ownPlanet']) {
            $this->galaxyData[$this->galaxyRow['planet']]['action'] = array(
				...
				'esp'		=> false,
                'message'	=> false,
				'fortress'	=> $this->galaxyRow['planet_type'] == 4 && $this->galaxyRow['capture_not'] > TIMESTAMP,
                'buddy'		=> false,
                'missle'	=> false,
                
            );
        }elseif($this->galaxyRow['id_owner'] == Fortress_Id || $this->galaxyRow['id_owner'] == Asteroid_Id) {
            $this->galaxyData[$this->galaxyRow['planet']]['action'] = array(
            ...
            $this->galaxyData[$this->galaxyRow['planet']]['action'] = array(
                'esp'		=> $USER['settings_esp'] == 1 && $USER['immunity_until'] < TIMESTAMP && $this->galaxyRow['immunity_until'] < TIMESTAMP && $this->galaxyData[$this->galaxyRow['planet']]['missions'][6],
                'message'	=> $USER['settings_wri'] == 1 && isModulAvalible(MODULE_MESSAGES) && $this->galaxyRow['planet_type'] != 4,
				'fortress'	=> $this->galaxyRow['planet_type'] == 4 && $this->galaxyRow['capture_not'] > TIMESTAMP,
                'buddy'		=> $USER['settings_bud'] == 1 && isModulAvalible(MODULE_BUDDYLIST) && $this->galaxyRow['buddy'] == 0 && $this->galaxyRow['planet_type'] != 4,
                'missle'	=> $USER['settings_mis'] == 1 && $USER['immunity_until'] < TIMESTAMP && $this->galaxyRow['immunity_until'] < TIMESTAMP && $this->galaxyData[$this->galaxyRow['planet']]['missions'][10],
            );
        }
	}
	...
	protected function getPlayerData()
	{
        ...
        $this->galaxyData[$this->galaxyRow['planet']]['user']	= array(
			...
			'isEnnemies'		=> $this->galaxyRow['ennemies'] == 0,
		);
	}
	...
	protected function getAllianceData()
	{
		...
		if(empty($this->galaxyRow['allyid'])) {
			...
			{
				case 1:
				$Class	= "style='color:#e0cbb0'";
				break;
				case 2:
					$Class	= "style='color:#389f43'";
				break;
				case 3:
					$Class	= "style='color:#bbad55'";
				break;
				case 4:
					$Class	="style='color:#b27a5f'";
				break;
				case 5:
					$Class	= "style='color:#b21f27'";
				break;
				case 6:
					$Class	= "style='color:#389f43'";
				break;
			...
			$this->galaxyData[$this->galaxyRow['planet']]['alliance']	= array(
				...
				'tag'		=> $this->galaxyRow['ally_tag'],				'fraction'		=> $this->galaxyRow['fraction'],
