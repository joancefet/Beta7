...
class ResourceUpdate
{
	...
	public function CreateHash() {
		...
		$Hash[]	= $this->USER['premium_reward_storing'];
		$Hash[]	= $this->USER['premium_reward_extraction'];
		$Hash[]	= $this->USER['experience_peace_level'];
		...
		$Hash[]	= $this->PLANET['metal_perhour'];
		$Hash[]	= $this->PLANET['crystal_perhour'];
		$Hash[]	= $this->PLANET['deuterium_perhour'];
		$Hash[]	= $this->PLANET['darkmatter_perhour'];
		$Hash[]	= $this->USER['combat_reward_collider'];
		$Hash[]	= $this->USER['academy_1201'];
		$Hash[]	= $this->USER['academy_1202'];
		$Hash[]	= $this->USER['academy_1204'];
		$Hash[]	= $this->USER['academy_1209'];
		$Hash[]	= $this->USER['academy_1210'];
		$Hash[]	= $this->USER['daily_produ'];
...
#		$this->CONF['energySpeed']	= 1;
	...
	private function ExecCalc()
	{
		...
		$MaxDarkmatterStorage	= 100000000000;
		...
		$DarkmatterTheorical	= $this->ProductionTime * ($this->PLANET['darkmatter_perhour']) / 3600;
		if ($DeuteriumTheorical < 0)
		{
			$this->USER['darkmatter']    = max($this->USER['darkmatter'] + $DarkmatterTheorical, 0);
		} 
		elseif($this->USER['darkmatter'] <= $MaxDarkmatterStorage) 
		{
			$this->USER['darkmatter']    = min($this->USER['darkmatter'] + $DarkmatterTheorical, $MaxDarkmatterStorage);
		}
		...
		}elseif($this->PLANET['planet_type'] == 1){
			
		$MaxMetalStorage		= $this->PLANET['metal_max']     * $this->CONF['max_overflow'];
		$MaxCristalStorage		= $this->PLANET['crystal_max']   * $this->CONF['max_overflow'];
		$MaxDeuteriumStorage	= $this->PLANET['deuterium_max'] * $this->CONF['max_overflow'];
		$MaxDarkmatterStorage	= 100000000;
		...
		$MetalTheorical		= $this->ProductionTime * (($this->CONF['metal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['metal_perhour']) / 3600 + ($this->ProductionTime * ((($this->CONF['metal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['metal_perhour']) / 3600) / 100 * $premium_resource) + ($this->ProductionTime * ((($this->CONF['metal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['metal_perhour']) / 3600) / 100 * $peacefull_resource) + ($this->ProductionTime * ((($this->CONF['metal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['metal_perhour']) / 3600) / 100 * $academy_mines) + ($this->ProductionTime * ((($this->CONF['metal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['metal_perhour']) / 3600) / 100 * $daily_prod_bonus);
		...
		$CristalTheorical	= $this->ProductionTime * (($this->CONF['crystal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['crystal_perhour']) / 3600 + ($this->ProductionTime * ((($this->CONF['crystal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['crystal_perhour']) / 3600) / 100 * $premium_resource)  + ($this->ProductionTime * ((($this->CONF['crystal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['crystal_perhour']) / 3600) / 100 * $peacefull_resource)  + ($this->ProductionTime * ((($this->CONF['crystal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['crystal_perhour']) / 3600) / 100 * $academy_mines) + ($this->ProductionTime * ((($this->CONF['crystal_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['crystal_perhour']) / 3600) / 100 * $daily_prod_bonus);
		if ($CristalTheorical < 0)
		{
			$this->PLANET['crystal']      = max($this->PLANET['crystal'] + $CristalTheorical, 0);
		} 
		...
		$DeuteriumTheorical	= $this->ProductionTime * (($this->CONF['deuterium_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['deuterium_perhour']) / 3600 + ($this->ProductionTime * ((($this->CONF['deuterium_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['deuterium_perhour']) / 3600) / 100 * $premium_resource) + ($this->ProductionTime * ((($this->CONF['deuterium_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['deuterium_perhour']) / 3600) / 100 * $peacefull_resource) + ($this->ProductionTime * ((($this->CONF['deuterium_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['deuterium_perhour']) / 3600) / 100 * $academy_mines) + ($this->ProductionTime * ((($this->CONF['deuterium_basic_income'] * $this->CONF['resource_multiplier']) + $this->PLANET['deuterium_perhour']) / 3600) / 100 * $daily_prod_bonus);
		if ($DeuteriumTheorical < 0)
		{
			$this->PLANET['deuterium']    = max($this->PLANET['deuterium'] + $DeuteriumTheorical, 0);
		} 
	...
	public static function getNetworkLevel($USER, $PLANET)
	{
		...
		if($USER[$resource[123]] == 0) {
			...
			{
				//$lablevel[]	= $Levels[$resource[31]] + $USER['academy_level_2_8'];
	...
	public function SetNextQueueElementOnTop()
	{
		...
		while ($Loop == true)
		{
			...
			if($HaveRessources === true) {
				...
				if(isset($costRessources[922])) { $this->USER[$resource[922]]	-= $costRessources[922]; }
	...
	public function SavePlanetToDB($USER = NULL, $PLANET = NULL)
	{
		...
		$Qry	= "LOCK TABLE ".PLANETS." as p WRITE, ".USERS." as u WRITE;
				   ...
				   p.darkmatter_perhour = ".$PLANET['darkmatter_perhour'].",
