...
function counting(id)
{
	...
	if(typeof Data.costRessources[922] !== "undefined")
	{
		res = Number(Data.costRessources[922]);
		costRessources[922] += Math.round(res * count);
	}
	...
	if(costRessources[922] > 0)
	{			
		Ellimet.find(".res922").find(".text").html(NumberGetHumanReadable(costRessources[922]));
		
		if(antimatter < costRessources[922])	
		{
			Ellimet.find(".res922").addClass('required'); 
			Ellimet.find(".res922").find(".text").addClass('tooltip'); 
			Ellimet.find(".res922").find(".text").attr('data-tooltip-content', LNGning + ' ' + LNGtech922 + ' ' + NumberGetHumanReadable(costRessources[922] - antimatter)); 
		}	
		else
		{
